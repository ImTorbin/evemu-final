...
void SuperWeapon::Activate(SystemEntity* pSE)
{
    ...
    if ((pSE == nullptr) or (m_shipRef->GetPilot() == nullptr) or (m_shipRef->GetPilot()->GetShipSE() == nullptr)) {
        Deactivate();
        return;
    }
...
void SuperWeapon::StopCycle(bool abort)
{
    ...
    ShipSE* pShipSE = (pPilot != nullptr ? pPilot->GetShipSE() : nullptr);
    if ((pPilot == nullptr) or (pShipSE == nullptr) or (pShipSE->DestinyMgr() == nullptr))
        return;
	...
	pShipSE->DestinyMgr()->SendSpecialEffect
	(
        m_shipRef,
        m_modRef->itemID(),
        m_modRef->typeID(),
    ...
    if ((m_destinyMgr == nullptr) or m_destinyMgr->IsWarping() or (m_bubble == nullptr))
...
double SuperWeapon::DoCycle()
{
        Client* pPilot = m_shipRef->GetPilot();
        ShipSE* pShipSE = (pPilot != nullptr ? pPilot->GetShipSE() : nullptr);
        if ((pPilot == nullptr) or (pShipSE == nullptr) or (m_targetSE == nullptr)
            || (!pShipSE->SysBubble())
            || (!pShipSE->SysBubble()->GetEntity(m_targetID))
