...
void TurretModule::ApplyDamage()
{
    ...
    if (m_chargeRef.get() == nullptr) {
        // catchall
        pPilot->SendErrorMsg("Your %s in %s doesnt have a charge registered.<br>You can try Unload/Reload, but if this error happens again, dock or relog.", \
            ...
            if (m_chargeRef->HasAttribute(AttrCrystalsGetDamaged))
                if (MakeRandomFloat(0,1) < m_crystalDmgChance) {
                    ...
                    if (m_crystalDmg > 1.0f) {
                        pPilot->SendNotifyMsg("Your %s loaded in %s has been destroyed.", m_chargeRef->name(), m_modRef->name());
