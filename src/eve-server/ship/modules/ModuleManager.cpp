...
void ModuleManager::Activate(int32 itemID, uint16 effectID, int32 targetID, int32 repeat)
{
    ...
    ShipSE* pShipSE = pShipItem->GetPilot()->GetShipSE();
    if (pShipSE == nullptr) {
        _log(PLAYER__ERROR, "%s: Pilot shipSE is null!", pShipItem->GetPilot()->GetName());
        return;
    }
    ...
    // Grouped turrets/launchers: only the master runs Activate(). Requests on a
    // slave slot were ignored, so the client showed weapons off while the
    // master never received Activate — or the user clicked a slave to stop and
    // hit the Deactivate no-op path below.
    if (pMod->IsLinked() && !pMod->IsMaster()
    && (pMod->IsTurretModule() || pMod->IsLauncherModule()))
        pMod = pShipItem->GetWeaponGroupMaster(pMod);
...
void ModuleManager::Deactivate(uint32 itemID, std::string effectName)
{
    ...
    // Slaves never enter Activated — only the master runs the firing cycle.
    // Route stop commands to the master so grouped weapons can actually cease fire.
    GenericModule* pActive = pMod;
    if (pMod->IsLinked() && !pMod->IsMaster()
    && (pMod->IsTurretModule() || pMod->IsLauncherModule()))
        pActive = pShipItem->GetWeaponGroupMaster(pMod);

    if (pActive->GetModuleState() != Module::State::Activated)  // we dont need an error msgs here....this is acceptable, as the module may not be active
        return;

    _log(MODULE__TRACE, "MM::Deactivate() - %s Deactivating - '%s'", pActive->GetSelf()->name(), effectName.c_str());
    pActive->Deactivate(effectName);
}
...
void ModuleManager::UnloadWeapons()
{
    ...
    for (uint8 i = flagHiSlot0; i < flagFixedSlot; ++i) {
        mItr = m_modules.find((uint8)i);
        if ((mItr != m_modules.end()) and (mItr->second != nullptr))
