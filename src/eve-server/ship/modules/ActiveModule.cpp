...
m_Stop(true)
{
    ...
    if (!m_shipRef->HasPilot())
        return;
    Client* pPilot = m_shipRef->GetPilot();
    if (pPilot == nullptr)
        return;
    ...
    switch (mRef->groupID()) {
        case EVEDB::invGroups::Ship_Scanner: {
            ...
            range *= (1 + (0.03f * (pChar->GetSkillLevel(EvESkill::LongRangeTargeting, true))));
...
void ActiveModule::Activate(uint16 effectID, uint32 targetID/*0*/, int16 repeat/*0*/)
{
    ...
    if (IsValidTarget(targetID)) {
        ...
        m_needsTarget = true;
        m_targetID = targetID;
        SystemManager* pSystem = pPilot->SystemMgr();
        if (pSystem == nullptr) {
            Clear();
            throw UserError ("DeniedActivateTargetNotPresent");
        }
        m_targetSE = pSystem->GetSE(targetID);
        if (m_targetSE == nullptr) {
            Clear();
            throw UserError ("DeniedActivateTargetNotPresent");
        }
    ...
    switch (groupID()) {
        ...
        case EVEDB::invGroups::Cloaking_Device: {
            // Engages the server-side cloak state machine. The module pointer
            // lets the DestinyManager abort this cycle when the cloak breaks
            // out-of-band (proximity, warp init abort via ShipWarping, etc.).
            m_destinyMgr->Cloak(this);
        } break;
    }
    /*def OnSpecialFX
...
void ActiveModule::DeactivateCycle(bool abort/*false*/)
{
    ...
    switch (groupID()) {
        ...
        case EVEDB::invGroups::Cloaking_Device: {
            // Mirror of the activate-side Cloak() call. Safe to invoke even
            // when the DestinyManager already cleared the cloak (e.g. via
            // proximity decloak which calls AbortCycle() on us first):
            // UnCloak() is idempotent and clears m_cloakMod before touching
            // the module pointer to break the abort-cycle recursion.
            if (m_destinyMgr != nullptr)
                m_destinyMgr->UnCloak();
        } break;
        case EVEDB::invGroups::Survey_Scanner: {
            if ((m_sysMgr == nullptr) or (m_bubble == nullptr) or (m_sysMgr->GetBeltMgr() == nullptr)) {
                Clear();
                return;
            }
            if (abort) {
...
void ActiveModule::UpdateCharge(uint16 attrID, uint16 testAttrID, uint16 srcAttrID, InventoryItemRef iRef)
{
    ...
    if (attrID == AttrShieldCharge) {
        Client* pPilot = m_shipRef->GetPilot();
        if ((pPilot != nullptr) and (pPilot->GetShipSE() != nullptr))
            pPilot->GetShipSE()->SendDamageStateChanged();
    }
}
...
void ActiveModule::ReprocessCharge()
{
    ...
    sFxProc.ApplyEffects(m_modRef.get(), pPilot->GetChar().get(), m_shipRef.get(), pPilot->IsInSpace());
    m_chargeRef->ClearModifiers();
}
...
void ActiveModule::ShowEffect(bool active/*false*/, bool abort/*false*/)
{
    ...
    if (!pPilot->IsInSpace())
        return;
    if (pPilot->GetShipSE() == nullptr)
        return;
    if (pPilot->GetShipSE()->SysBubble() == nullptr)
        return;

    if (m_effectID < 1) {
        _log(EFFECTS__ERROR, "fxID = 0 for %s.", m_modRef->name());
        m_effectID = m_modRef->type().GetDefaultEffect();
        if (m_effectID < 1) {
            _log(EFFECTS__ERROR, "No valid fallback effect for %s. Skipping ShowEffect to avoid invalid OnGodmaShipEffect payload.", m_modRef->name());
            return;
        }
    }
    ...
    or   m_destinyMgr->IsWarping()) {
        pPilot->QueueDestinyEvent(&tuple);
    } else {
...
void ActiveModule::LaunchMissile()
{
    ...
    if (pMissile->DestinyMgr() != nullptr)
...
void ActiveModule::LaunchProbe()
{
    ...
    if (pChar == nullptr)
        return;
    if (pcount > (pChar->GetSkillLevel(EvESkill::Astrometrics) + 3)) {
