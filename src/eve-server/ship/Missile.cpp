...
void Missile::HitTarget() {
    ...
    // Defensive clamps. Missing/malformed attributes on civilian/defender/snowball/bomb
    // charges can otherwise produce NaN or Inf in the damage formula and blow up shields
    // for (base damage * inf) or apply zero damage silently.
    if (Er < 0.01)  Er  = 0.01;               // avoid div-by-zero in Sr/Er
    if (Ev < 0.01)  Ev  = 0.01;               // avoid (Ev=0 -> 0 * anything -> pow(0, exp) = 0, unintended)
    if (DRF <= 1.0) DRF = 3.0;                // stock EVE default when reduction-factor is wrong
    if (DRS <= 1.0) DRS = 5.5;                // stock EVE default; DRS == 1 divides by log(1) = 0

    // V is the target's ABSOLUTE speed (EVE missile formula uses speed, not transversal).
    // Clamp to a small positive value so (Ev/V) is bounded for stationary targets.
    GVector Vel = m_targetSE->GetVelocity();
    double V = Vel.length();
    if (V < 1.0)
        V = 1.0;
    ...
    if (!std::isfinite(v2))
        v2 = 1.0;   // worst case for the target: full damage (other terms still clamp via min1)

    // apply damage modifier from char skills, if applicable
    d *= m_damageMod;
    // apply missile damage formula to computed total damage:
    //  Damage *= MIN(1, Sr/Er, ((Ev/V)*(Sr/Er))^(ln(DRF)/ln(DRS)))
    d *= EvE::min1(v1, v2);
