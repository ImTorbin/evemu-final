...
private:
    /* Shared pre-flight used by both single-ship and fleet-warp entry points.
     * Returns false and sends the appropriate notify/error to the client if
     * the client cannot initiate a warp right now (no destiny, already warping,
     * frozen, login-warping, or warp-scrambled). */
    bool CanIssueWarp(Client* issuer);

    /* Dispatches a fleet warp from issuer. Role-scoped via FleetService:
     *   FleetLeader = whole fleet, WingLeader = wing, SquadLeader = squad.
     * includeIssuer controls whether the issuer's own ship is warped too.
     * regroup selects the FleetRegroup notify string vs the FleetWarp one. */
    bool DoFleetWarp(Client* issuer, const GPoint& where, int32 distance,
