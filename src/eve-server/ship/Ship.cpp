...
{
    // Map is keyed by the group master only; resolve slave -> master so a
    // client addressing any grouped slot still works.
    GenericModule* master = GetWeaponGroupMaster(pMod);
    std::map<GenericModule*, std::list<GenericModule*>>::iterator itr = m_linkedWeapons.find(master);
    if (itr == m_linkedWeapons.end())
        return;

    const int size = static_cast<int>(chargeIDs.size());
    if (size < 1)
        return;
    ...
    // Master uses itemIDs[0].  Without bumping pos before the slave loop, every
    // slave would reload itemIDs[0], deleting / corrupting ammo for all slots.
    VerifyHoldType(master->flag(), cRef, m_pilot);
    m_ModuleManager->LoadCharge(cRef, master->flag());
    ++pos;

    for (auto itr2 = itr->second.begin(); itr2 != itr->second.end() && pos < size; ++itr2) {
        cRef = sItemFactory.GetItemRef(chargeIDs[pos]);
        ++pos;
        if (cRef.get() == nullptr)
            continue;
...
GenericModule* ShipItem::GetWeaponGroupMaster(GenericModule* pMod)
{
    if (pMod == nullptr || !pMod->IsLinked())
        return pMod;

    auto itr = m_linkedWeapons.find(pMod);
    if (itr != m_linkedWeapons.end())
        return pMod;

    for (auto& cur : m_linkedWeapons) {
        for (GenericModule* slave : cur.second)
            if (slave == pMod)
                return cur.first;
    }
    return pMod;
...
{
    pMod = GetWeaponGroupMaster(pMod);
    uint8 count = 0;
    auto itr = m_linkedWeapons.find(pMod);
    if (itr == m_linkedWeapons.end()) {
        if (pMod->IsLoaded())
    ...
    if (pMod->IsLoaded())
        ++count;
    for (GenericModule* slave : itr->second)
        if (slave->IsLoaded())
            ++count;
    return count;
}
...
PyDict* ShipSE::MakeSlimItem() {
    ...
    PyDict *slim = new PyDict();
        ...
        // Crucible michelle uses ballID to match slims to ClientBalls; without it, clients may see ballID=0 and break park state.
...
bool ShipSE::LaunchDrone(InventoryItemRef dRef) {
    ...
    if ((pPilot == nullptr) || (dRef.get() == nullptr))
        return false;
    ...
    /*
     * Validate bandwidth first. Previously we spawned/launched drones and only
     * then failed this check, which left inert drones in space when launch was
    ...
    m_drones.emplace(pDrone->GetID(), dRef.get());
    ...
    */
    // bandwidth was validated before launch; bring drone online immediately
    pDrone->Online();
...
void ShipSE::ScoopDrone(SystemEntity* pSE) {
    ...
    m_drones.erase(pSE->GetSelf()->itemID());
