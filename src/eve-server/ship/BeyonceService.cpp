...
bool BeyonceBound::CanIssueWarp(Client* issuer)
{
    if (issuer == nullptr)
        return false;
    ...
    SystemEntity* pShipSE = issuer->GetShipSE();
    if (pShipSE == nullptr) {
        codelog(CLIENT__ERROR, "%s: Client has no ship SE!", issuer->GetName());
        return false;
    }

    DestinyManager* pDestiny = pShipSE->DestinyMgr();
    if (pDestiny == nullptr) {
        codelog(CLIENT__ERROR, "%s: Client has no destiny manager!", issuer->GetName());
        return false;
    }
    if (pDestiny->IsWarping()) {
        issuer->SendNotifyMsg("You are already warping");
        return false;
    }
    if (pDestiny->AbortIfLoginWarping(true))
        return false;
    if (pDestiny->IsFrozen()) {
        issuer->SendNotifyMsg("Your ship is frozen and cannot move");
        return false;
    }
    return true;
}

bool BeyonceBound::DoFleetWarp(Client* issuer, const GPoint& where, int32 distance,
                               bool includeIssuer, bool regroup, SystemEntity* pSE /*nullptr*/)
{
    if (issuer == nullptr)
        return false;

    SystemEntity* pIssuerSE = issuer->GetShipSE();
    if (pIssuerSE == nullptr)
        return false;

    // Fetch role-scoped targets up front: FC -> fleet, WC -> wing, SC -> squad.
    std::vector<Client*> targets;
    const bool hasAuthority = sFltSvc.GetFleetWarpTargets(issuer, targets);
    if (!hasAuthority) {
        // CanIssueWarp already ran; this is purely a role/fleet-state check.
        issuer->SendErrorMsg("You must be a fleet, wing, or squad commander to issue a fleet warp.");
        return false;
    }

    // Issuer warps first. Keep the existing invul/undock clearing identical to
    // the self-warp path so players in command see no behavioral difference.
    if (includeIssuer) {
        DestinyManager* pDestiny = pIssuerSE->DestinyMgr();
        if (pDestiny != nullptr) {
            issuer->SetInvul(false);
            issuer->SetUndock(false);
            pDestiny->WarpTo(where, distance, false, pSE);
        }
    }

    // Only subordinates that share the issuer's bubble/grid get pulled in;
    // this matches live EVE behavior and prevents accidentally ordering
    // warps on ships the FC can't actually see.
    const uint16 issuerBubbleID = (pIssuerSE->SysBubble() != nullptr)
                                  ? pIssuerSE->SysBubble()->GetID() : 0;
    const char* issuerName = issuer->GetName();

    for (Client* sub : targets) {
        if (sub == nullptr || sub == issuer)
            continue;
        if (!sub->IsInSpace())
            continue;

        SystemEntity* pSubSE = sub->GetShipSE();
        if (pSubSE == nullptr)
            continue;

        // Same grid as issuer.
        if (pSubSE->SysBubble() == nullptr
        ||  pSubSE->SysBubble()->GetID() != issuerBubbleID)
            continue;

        DestinyManager* pSubDestiny = pSubSE->DestinyMgr();
        if (pSubDestiny == nullptr)
            continue;

        // Skip subs that can't accept the warp right now. These are soft
        // failures: the rest of the fleet must still warp.
        if (pSubDestiny->IsWarping() || pSubDestiny->IsFrozen())
            continue;
        if (pSubDestiny->AbortIfLoginWarping(false))
            continue;

        // Per-ship warp scramble: warp-scrambled subs silently stay put;
        // DestinyManager::WarpTo will also do its own cap / min-distance checks.
        ShipItemRef subShip = sub->GetShip();
        if (subShip && subShip->GetAttribute(AttrWarpScrambleStatus) > 0) {
            sub->SendNotifyMsg("You are warp scrambled and cannot follow the fleet warp.");
            continue;
        }

        sub->SetInvul(false);
        sub->SetUndock(false);
        pSubDestiny->WarpTo(where, distance);

        if (regroup) {
            sub->SendNotifyMsg("Regrouping to %s", issuerName);
        } else {
            sub->SendNotifyMsg("Following %s in warp", issuerName);
        }
    }
...
PyResult BeyonceBound::CmdWarpToStuff(PyCallArgs &call, PyString* type, PyRep* id) {
  ...
  _log(SERVICE__CALL_DUMP, "BeyonceBound::Handle_CmdWarpToStuff() - size=%lli", call.tuple->size());
   call.Dump(SERVICE__CALL_DUMP);
    ...
    // Fleet warp is only allowed for FC / WC / SC; reject early so a member
    // who triggered the menu item sees a proper error instead of silent warp.
    if (fleet) {
        const int8 role = call.client->GetFleetRole();
        if (!call.client->InFleet()
         || (role != Fleet::Role::FleetLeader
          && role != Fleet::Role::WingLeader
          && role != Fleet::Role::SquadLeader)) {
            call.client->SendErrorMsg("You must be a fleet, wing, or squad commander to issue a fleet warp.");
            return PyStatic.NewNone();
        }
    }
    ...
    // [warpfleettomember]   char, charid, minrange, fleet=1
        if (!call.client->InFleet()) {
            call.client->SendErrorMsg("You are not a member of any fleet.");
            return PyStatic.NewNone();
        }

        // Resolve the target client by character ID within the caller's fleet.
        Client* pTargetClient = nullptr;
        std::vector<Client*> fleetClients = sFltSvc.GetFleetClients(call.client->GetFleetID());
        for (Client* c : fleetClients) {
            if (c != nullptr && c->GetCharacterID() == toID) {
                pTargetClient = c;
                break;
            }
        }
        if (pTargetClient == nullptr) {
            call.client->SendErrorMsg("This person is either not a member of your fleet or not present in this solar system.");
            return PyStatic.NewNone();
        }
        if (!pTargetClient->IsInSpace()
         ||  pTargetClient->GetShipSE() == nullptr
         ||  pTargetClient->GetShipSE()->SystemMgr() == nullptr
         ||  pTargetClient->GetShipSE()->SystemMgr()->GetID() != pSystem->GetID()) {
            call.client->SendErrorMsg("Fleet member is not in this system.");
            return PyStatic.NewNone();
        }

        // target ship itself is the warp-to entity; reuse standard entity
        // handling below so ship radius / stop-distance math applies.
        pSE = pTargetClient->GetShipSE();
    } else {
    ...
    if (fleet) {
        // Fleet warp: issuer + role-scoped subordinates in the same system/grid.
        DoFleetWarp(call.client, warpToPoint, distance,
                    /*includeIssuer*/ true, /*regroup*/ false, pSE);
    } else {
...
PyResult BeyonceBound::CmdFleetRegroup(PyCallArgs &call) {
    /* Regroup: role-scoped fleet warps onto the issuer's current position.
     * The issuer does not move; eligible subordinates in the same system +
     * grid are warped to (roughly) where the issuer is right now.
     */
    ...
    if (!call.client->InFleet()
     || (role != Fleet::Role::FleetLeader
      && role != Fleet::Role::WingLeader
      && role != Fleet::Role::SquadLeader)) {
        call.client->SendErrorMsg("You must be a fleet, wing, or squad commander to order a regroup.");
        return PyStatic.NewNone();
    }

    SystemEntity* pSelfSE = call.client->GetShipSE();
    if (pSelfSE == nullptr)
        return PyStatic.NewNone();

    // Small non-zero distance so subordinates don't try to land on top of the
    // leader's ship (and to stay above the min-warp-distance floor for
    // anyone sitting very close to the leader already).
    DoFleetWarp(call.client, pSelfSE->GetPosition(), 2500,
                /*includeIssuer*/ false, /*regroup*/ true);
    return PyStatic.NewNone();
}
