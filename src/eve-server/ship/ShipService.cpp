...
BoundDispatcher *ShipService::BindObject(Client *client, PyRep* bindParameters) {
    ...
    ShipItemRef activeShip = client->GetShip();
    if (activeShip.get() == nullptr) {
        codelog(CLIENT__ERROR, "%s: ShipService bind requested with no active ship.", client->GetName());
        return nullptr;
    }
    uint32 shipID = activeShip->itemID();
    auto it = this->m_instances.find (shipID);

    if (it != this->m_instances.end ())
        return it->second;

    ShipBound* bound = new ShipBound(this->GetServiceManager(), *this, activeShip.get());
...
PyResult ShipBound::Board(PyCallArgs &call, PyInt* newShipID, std::optional<PyInt*> oldShipID) {
    ...
    SystemEntity* pTargetSE = pSystem->GetSE(newShipID->value());
    if (pTargetSE == nullptr) {
        _log(SHIP__ERROR, "Handle_Board() - Failed to find target entity %u for %s.", newShipID->value(), pClient->GetName());
        throw CustomError ("Unable to locate that ship in space.");
    }
    if (!pTargetSE->IsShipSE()) {
        _log(SHIP__ERROR, "Handle_Board() - Target entity %u is not a ship for %s.", newShipID->value(), pClient->GetName());
        throw CustomError ("You can only board ships.");
    }
    pShipSE = pTargetSE->GetShipSE();
    ...
    for (uint32 i = 0; i < PyToDropList->size(); ++i) {
        ...
        switch (iRef->categoryID()) {
            ...
            case EVEDB::invCategories::Structure: {
                ...
                SystemEntity* closestMoonSE = pClient->SystemMgr()->GetClosestMoonSE(location);
                if ((closestMoonSE == nullptr) or (!closestMoonSE->IsMoonSE())) {
                    pClient->SendErrorMsg("Unable to determine nearest moon.  Aborting Drop.");
                    return nullptr;
                }
                if (iRef->groupID() == EVEDB::invGroups::Control_Tower) {
                    if (closestMoonSE->GetMoonSE()->HasTower()) {
                ...
                dropped = true;
                shipDrop = true;
                if ((pSE->IsPOSSE()) and (pClient->GetShipSE() != nullptr))
            ...
            case EVEDB::invCategories::SovereigntyStructure: {
                ...
                switch (iRef->groupID()) {
                    case EVEDB::invGroups::Sovereignty_Blockade_Units: {
                        ...
                        if ((pSE != nullptr) and pSE->IsGateSE() and pSE->GetGateSE()->HasSBU()) {
            ...
            case EVEDB::invCategories::Celestial: { //Outpost construction platforms
                ...
                if (iRef->groupID() == EVEDB::invGroups::Secure_Cargo_Container or
                    iRef->groupID() == EVEDB::invGroups::Cargo_Container or
                    iRef->groupID() == EVEDB::invGroups::Freight_Container or
                    iRef->groupID() == EVEDB::invGroups::Audit_Log_Secure_Container or
                    iRef->groupID() == EVEDB::invGroups::Mission_Container)
                {
                    DBSystemDynamicEntity entity = DBSystemDynamicEntity();
                    entity.ownerID = ownerID;
                    entity.factionID = pClient->GetWarFactionID();
...
PyResult ShipBound::Jettison(PyCallArgs &call, PyList* itemIDs) {
    ...
    ShipSE* pShipSE = pClient->GetShipSE();
    if (pShipSE == nullptr) {
        codelog(CLIENT__ERROR, "%s: Client has no shipSE.", pClient->GetName());
        return nullptr;
    }
    ...
    for (; itr != ints.end(); ++itr) {
        ...
        switch (iRef->categoryID()) {
            ...
            case EVEDB::invCategories::StructureUpgrade: {
                ...
                if (sRef.get() == nullptr)
                    throw CustomError ("Unable to spawn Structure item %u.", *itr);
            ...
            case EVEDB::invCategories::Deployable: {
                ...
                if (cRef.get() == nullptr)
                    throw CustomError ("Unable to spawn Deployable item %u.", *itr);
                ...
                switch (iRef->groupID()) {
                    ...
                    case EVEDB::invGroups::Freight_Container: {
                        ...
                        if (ccRef.get() == nullptr)
                            throw CustomError ("Unable to spawn cargo container item %u.", *itr);
