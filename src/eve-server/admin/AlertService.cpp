...
PyResult AlertService::SendClientStackTraceAlert(PyCallArgs &call, PyTuple* stackId, PyString* stackTrace, PyString* mode, PyRep* nextErrorKeyHash) {
    bool isLegacyTickerTlsError = false;
    if (stackTrace != nullptr) {
        const std::string& trace = stackTrace->content();
        if ((trace.find("holoscreensvc.py") != std::string::npos)
        and (trace.find("GetNewsTickerData") != std::string::npos)
        and (trace.find("sslv3 alert handshake failure") != std::string::npos))
            isLegacyTickerTlsError = true;
    }

    if (isLegacyTickerTlsError) {
        _log(CLIENT__INFO, "AlertService::SendClientStackTraceAlert(): ignoring known Crucible news ticker TLS handshake failure.");
        return PyStatic.NewNone();
    }
