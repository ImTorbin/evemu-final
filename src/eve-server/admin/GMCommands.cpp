...
PyResult Command_giveallskills(Client* who, CommandDB* db, EVEServiceManager &services, const Seperator& args) {
    ...
    if (character.get()) {
        ...
        if (!CommandDB::NotFullyLearnedSkillList(skillList, ownerID))
            throw CustomError("Database error: could not load skill list for character %u.", ownerID);
        ...
        for (auto cur: skillList) {
            skillID = cur.first;
            bool needInject = (cur.second < 0);
            if (!needInject) {
                skill = character->GetCharSkillRef(skillID);
                if (skill.get() == nullptr)
                    needInject = true; // DB said skill exists but inventory is out of sync — inject
                else {
            ...
            if (needInject) {
                ...
                if (skill.get() == nullptr)
                    throw CustomError("Unable to create skill typeID %u (invalid type or spawn failure).", skillID);
...
PyResult Command_giveskills(Client* who, CommandDB* db, EVEServiceManager &services, const Seperator& args) {
    return Command_giveskill(who, db, services, args);
}
...
PyResult Command_giveskill(Client* who, CommandDB* db, EVEServiceManager &services, const Seperator& args) {
    ...
    if (args.argCount() == 3) {
        ...
        if (args.isNumber(1)) {
            ...
            pTarget = sEntityList.FindClientByCharID(ownerID);
            if (pTarget == nullptr)
                throw CustomError("Character %u is not online. Grant skills only while the character is logged in.", ownerID);
            character = pTarget->GetChar();
        } else if (args.arg(1) == "me") {
            ownerID = who->GetCharacterID();
            character = who->GetChar();
