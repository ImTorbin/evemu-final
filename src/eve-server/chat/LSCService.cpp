...
PyResult LSCService::SendMessage(PyCallArgs& call, PyRep* channelInfo, PyWString* cMessage)
{
    ...
    if ((call.tuple->IsTuple()) and (!call.tuple->AsTuple()->items.empty()) and (call.tuple->AsTuple()->items[0]->IsInt())) {
    ...
    // Some client builds throw when clicking fitting links (fittingSvc -> missing godma).
    // Until fitting-link support is fully stable, de-link fitting URLs in chat so clicking
    // no longer triggers the crashing path.
    if (message.find("fitting:") != std::string::npos) {
        std::string::size_type pos = 0;
        while ((pos = message.find("<url=fitting:", pos)) != std::string::npos) {
            message.replace(pos, 13, "[fitting:");
            pos += 9;
        }
        pos = 0;
        while ((pos = message.find("</url>", pos)) != std::string::npos) {
            message.replace(pos, 6, "]");
            pos += 1;
        }
        _log(LSC__WARNING, "Stripped fitting links from chat message for %s to avoid client fittingSvc crash.", call.client->GetName());
    }

    if (!message.empty() and message.at(0) == '.') {
