...
void Client::WarpIn() {
    ...
    /* Queue LoginWarp on an almost-immediate timer tick.  A bare SetStateTimer(0) is wrong:
     * it becomes SetStateTimer(0, Player::Timer::Default) — invalid state 0 and 1000ms delay. */
    SetStateTimer(Player::State::LoginWarp, 1);
}
...
void Client::MoveToLocation(uint32 locationID, const GPoint& pt) {
    ...
    if (sDataMgr.IsStation(m_locationID)) {
        ...
        */
        ...
        // After stargate/WH/drive jump, refresh ship slim + destiny ship vars so
        // the client repopulates hull stats (warp bubble collapse range, etc.).
        if (IsJump())
            UpdateNewShip();
    }
...
void Client::SetInvulTimer(uint32 time/*Player::Timer::Default*/)
{
    ...
    if (m_invulTimer.Enabled()) {
        /* WarpInInvul from login may still be running when we undock and need UndockInvul.
         * Docked clients may also never run ProcessClient's invul Check(), leaving 0ms remaining
         * but Enabled. Always replace with the new duration instead of failing the call. */
        const uint32 remaining = m_invulTimer.GetRemainingTime();
        if (remaining == 0)
            _log(CLIENT__TIMER, "%s: Invul Timer was expired but still enabled (e.g. docked); replacing.", m_char->name());
        else if (is_log_enabled(CLIENT__TIMER))
            _log(CLIENT__TIMER, "%s: Invul Timer replaced (%ums remaining → %ums).", m_char->name(), remaining, time);
        m_invulTimer.Disable();
        SetInvul(false);
    }
...
void Client::_SendQueuedUpdates() {
    /* Drop stale subwarp packets (e.g. queued in space then docked; or session still "station" client-side). */
    if (sDataMgr.IsStation(m_locationID) || pShipSE == nullptr || !m_setStateSent) {
...
// True only while `LoginWarp` state is active (after `WarpIn()` queued the warp).
// Do NOT include `Player::State::Login`: that is only the ~4s post-select timer
// before `WarpIn()` runs; treating it as "login warping" blocks all Beyonce nav
// with "You cannot perform this action right now." for no gameplay reason.
//
// Do not OR in m_loginWarpPoint / m_loginWarpRandomPoint: those are cleared in
// SetLoginWarpComplete(), but m_clientState can become Idle earlier without clearing
// them (e.g. SetBallPark jump paths). Stale points would then block forever if we
// keyed off coordinates.
//
// Destiny should call SetLoginWarpComplete() when the login warp finishes or aborts.
bool Client::IsLoginWarping() {
    return m_clientState == Player::State::LoginWarp;
}
