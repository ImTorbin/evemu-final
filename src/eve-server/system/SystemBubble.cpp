...
void SystemBubble::Add(SystemEntity* pSE) {
    ...
    // Leaving another bubble (warp, slow grid crossing): unregister there first
    // so we are not listed in two bubbles. Remove() notifies the old bubble with
    // RemoveBall when appropriate (e.g. ship arrived at a new grid).
    if (pSE->m_bubble != nullptr && pSE->m_bubble != this) {
        pSE->m_bubble->Remove(pSE);
    }
