...
mvPacket(nullptr)
{
    ...
    m_orbitTime = 0.0f;
    m_orbitRadTic = 0.0f;
    m_orbitPhaseOffsetRad = 0.0f;
    m_orbitPhaseOriginMS = 0;
    m_timeFraction = 0.0f;
    ...
    m_lastSelfSyncMS = 0;
    m_lastCloakProxCheck = 0;
    m_cloakMod = nullptr;
    m_preCloakMaxSpeed = 0.0f;

    m_orbitPhase = DestinyOrbitPhase::None;
    m_orbitHasPrevTargetPos = false;
}
...
bool DestinyManager::IsMoving() {
    const double vx = m_velocity.x * m_velocity.x + m_velocity.y * m_velocity.y + m_velocity.z * m_velocity.z;
    if (vx > 4.0)
        return true;
    if (m_activeSpeedFraction > 0.002f)
        return true;
    if (m_userSpeedFraction > 0.002f)
        return true;
    return (m_timeFraction > 0.002f);
}

float DestinyManager::GetSpeed()
{
    const double vx = double(m_velocity.x) * m_velocity.x + double(m_velocity.y) * m_velocity.y
        + double(m_velocity.z) * m_velocity.z;
    if (vx > 1.0)
        return static_cast<float>(std::sqrt(vx));
    if (m_maxShipSpeed > 0.001f)
        return m_maxShipSpeed * m_activeSpeedFraction;
    return 0.f;
}

float DestinyManager::GetSpeedFraction()
{
    if (m_maxShipSpeed > 0.001f) {
        const float f = GetSpeed() / m_maxShipSpeed;
...
void DestinyManager::Process() {
    ...
    /* Piloted ships: subwarp/orbit/stop motion runs in ProcessHighFreqState() at DynamicDestinyMs
     * with wall-clock dt in MoveObject — same path as NPCs. (Previously only 1Hz here, observers
     * extrapolated from sparse snaps and diverged badly from server GetAuthPosition.)
     * In-warp cruise (m_warpState set) still uses stamp-based ProcessState once per second —
     * warp accel math keys off EntityList GetStamp(). Missiles stay 1Hz-only. */
    if (mySE->HasPilot()) {
        if (m_ballMode == Destiny::Ball::Mode::WARP && m_warpState != nullptr)
            ProcessState();
    } else if (m_ballMode == Destiny::Ball::Mode::MISSILE
        || (m_ballMode == Destiny::Ball::Mode::WARP && m_warpState != nullptr)) {
        ProcessState();
    }

    // Proximity decloak check. Throttled so a cloaked ship in a crowded
    // bubble doesn't burn a full O(N) scan every tick (destiny is ~5 Hz).
    // A 1s re-evaluation is well under the client-visible decloak reaction
    // window and matches CCP's observed server-tick granularity.
    if (m_cloaked && (m_cloakMod != nullptr)) {
        const uint32 stamp = sEntityList.GetStamp();
        if (stamp - m_lastCloakProxCheck >= 1) {
            m_lastCloakProxCheck = stamp;
            CheckCloakProximity();
        }
    }
...
void DestinyManager::ProcessHighFreqState() {
    if (mySE->IsFrozen()) {
        Halt();
        return;
    }
    /* Pilots: integrate subwarp here. Skip in-warp tunnel (stamp-based physics in Process). */
    ...
    switch(m_ballMode) {
        ...
        case Ball::Mode::WARP: {
            ...
            if ((degrees < WARP_ALIGNMENT) and (m_timeFraction > 0.749)) {
                ...
                // catchall for turn checks messed up, and m_moveTime > ship align time (still enters warp via InitWarp())
                if (mySE->HasPilot()) {
                    _log(DESTINY__WARNING, "Destiny::ProcessState() Error!  Ship %s(%u) for Player %s(%u) - warp align/speed is incorrect, but time > shipTimeToWarp.",  \
                                mySE->GetName(), mySE->GetID(), mySE->GetPilot()->GetName(), mySE->GetPilot()->GetCharacterID());
                } else {
                    _log(DESTINY__WARNING, "Destiny::ProcessState() Error!  NPC %s(%u) - warp align/speed is incorrect, but time > shipTimeToWarp.",  \
                            mySE->GetName(), mySE->GetID());
                }
                m_shipHeading = toVec;
                InitWarp();
...
void DestinyManager::SetSpeedFraction(float fraction/*1.0*/, bool startMovement/*false*/) {
    ...
    if (m_orbiting != 0) {
        if (m_targetEntity.second != nullptr) {
            ...
            // Halt() and some paths clear the target without ClearOrbit(); don't call Orbit(nullptr).
            _log(DESTINY__WARNING, "Destiny::SetSpeedFraction() - %s(%u): m_orbiting=%u but orbit target is null; clearing orbit.",
                mySE->GetName(), mySE->GetID(), m_orbiting);
            ClearOrbit();
        }
    }
    ...
    // Any non-zero speed command after Stop() should immediately leave stop state.
    // Without this, m_stop can stay true during decel and user re-accel requests are ignored,
    // causing visible client/server divergence (rubber-band when the server snaps back).
    if (m_stop && fraction > 0.001f)
        startMovement = true;
    ...
    // Only drop WARP→GOTO when exiting the warp *tunnel* (WarpStop passes
    // fraction==0 while m_warpState still exists — see WarpStop()). Doing this
    // on warp *entry* (align phase: WARP && m_warpState==nullptr) skipped
    // ProcessState's WARP branch entirely, so InitWarp() never ran: ship stayed
    // in broken GOTO/subwarp math (log spam "decel = true, but psf = 0") and
    // bubbles/overview thrashed on login.
    if (m_ballMode == Destiny::Ball::Mode::WARP) {
        if (m_warpState != nullptr && fraction == 0.0f)
            m_ballMode = Destiny::Ball::Mode::GOTO;
        return;
    }
...
bool DestinyManager::AbortIfLoginWarping(bool showMsg) {
    ...
    // Only block while we are in active login warp state *and* destiny is
    // actually in warp mode. This avoids false "cannot perform action" popups
    // when LoginWarp is merely queued (state timer tick pending) or stale.
    if (pClient != nullptr && pClient->IsLoginWarping() && IsWarping()) {
        if (showMsg) {
            _log(
                CLIENT__WARNING,
                "AbortIfLoginWarping: nav blocked for %s(%u) — player state %s (finish login warp or see SetLoginWarpComplete).",
                pClient->GetName(),
                pClient->GetCharacterID(),
                pClient->GetStateName(pClient->GetPlayerState()).c_str()
            );
...
void DestinyManager::Stop() {
    ...
    if (m_userSpeedFraction == 0.0f) {
        m_stop = true;
    } else if ((m_ballMode == Destiny::Ball::Mode::WARP) and (m_warpState == nullptr)) {
        // warp aborted while still aligning (before InitWarp).  standard Stop() applies.
        m_ballMode = Destiny::Ball::Mode::STOP;
    } else if (IsMoving()) {
    ...
    // CmdStop tells the client to decelerate on its own timeline (matching the server's
    // physics constants). It MUST be sent on every Stop() call -- SetSpeedFraction(0) does
    // NOT emit a CmdSetSpeedFraction packet (its send is gated on `if (fraction)`), so
    // without this the client would never learn it's supposed to stop and would keep
    // cruising at the last commanded speed. This is broadcast to the whole bubble so
    // remote observers also stop predicting forward motion for this ball.
    CmdStop du;
        du.entityID = mySE->GetID();
...
void DestinyManager::Halt() {
    ...
    ClearTurn();
    // Must match Stop(): otherwise m_orbiting stays set and SetSpeedFraction() calls Orbit(nullptr).
    ClearOrbit();
    m_lastNpcMoveIntegrateMS = 0;
    m_lastNpcGotoFollowBroadcastMS = 0;
...
void DestinyManager::CheckBump()
{
    ...
    thread_local std::vector<Client*> vPlayers;
    vPlayers.clear();
    ...
    for (auto cur : vPlayers) {
        ...
        if (!cur->IsInSpace() || (cur->GetShipSE() == nullptr))
            continue;
        distance = pos.distance(cur->GetShipSE()->GetAuthPosition());
...
void DestinyManager::MoveObject() {
    ...
    if ((timeStamp > m_shipAccelTime) and (m_timeFraction > 0.9998f)) {
        const float activeBeforeSteady = m_activeSpeedFraction;
        ...
        if (m_userSpeedFraction) {
            ...
            // ship has reached full commanded speed — lock TF so the next tic still
            // satisfies the steady-state guard (otherwise m_timeFraction stays stale from
            // the accel curve and we fall into the partial branch with Acc/Dec false → error spam).
            m_timeFraction = 1.0f;
            move = "at constant speed, going";
        } else if (activeBeforeSteady > 0.001f) {
            /* Steady-state guard matched with USF==0 but we still carry speed — do not Halt():
             * assigning ASF:=USF here would zero velocity while the curve is not finished and
             * observers rubber-band. Continue deceleration from the last known fraction. */
            m_accel = false;
            m_decel = true;
            if (m_prevSpeedFraction < 0.001f)
                m_prevSpeedFraction = activeBeforeSteady;
            m_activeSpeedFraction = m_prevSpeedFraction - (m_prevSpeedFraction - m_userSpeedFraction) * m_timeFraction;
            speed = m_maxShipSpeed * m_activeSpeedFraction;
            m_timeFraction = 1.0f;
            move = "decelerating (steady-end recover)";
        } else {
            m_activeSpeedFraction = m_userSpeedFraction;
            speed = m_maxShipSpeed * m_activeSpeedFraction;
        ...
        if (m_accel) {
            ...
            move = m_turning ? "accelerating in turn" : "accelerating";
            ...
            move = m_turning ? "decelerating for turn" : "decelerating";
            if (m_prevSpeedFraction) {
                // asf = psf - (psf - usf) * tf
                m_activeSpeedFraction = m_prevSpeedFraction - (m_prevSpeedFraction - m_userSpeedFraction) * m_timeFraction;
            } else {
                // Inconsistent decel without PSF (e.g. movement flags out of sync). Recover as accel-from-stop.
                m_decel = false;
                if (m_userSpeedFraction > 0.001f)
                    m_accel = true;
                m_activeSpeedFraction = m_userSpeedFraction * m_timeFraction;
            }
        } else if (m_tractored or m_tractorPause) {
            ;   // do nothing here.  this is to remove error reporting from next line.
        } else if (m_userSpeedFraction > 0.001f) {
            /* Stable cruise after accel/decel flags were cleared but the steady-state
             * guard above didn't match (stale m_timeFraction / timing). Same as full speed. */
            m_timeFraction = 1.0f;
            m_activeSpeedFraction = m_userSpeedFraction;
            move = "coasting";
        } else if (m_userSpeedFraction < 0.001f && m_activeSpeedFraction > 0.001f) {
            /* Stopping or decel flags were lost while still moving — never Halt() here:
             * Halt() zeros state and snaps clients; continue the stop curve instead. */
            m_decel = true;
            if (m_prevSpeedFraction < 0.001f)
                m_prevSpeedFraction = m_activeSpeedFraction;
            m_activeSpeedFraction = m_prevSpeedFraction - (m_prevSpeedFraction - m_userSpeedFraction) * m_timeFraction;
            move = "decelerating (recovered)";
        } else if (m_activeSpeedFraction < 0.001f && m_userSpeedFraction < 0.001f) {
            Halt();
            return;
        } else {
            if (is_log_enabled(DESTINY__WARNING))
                _log(DESTINY__WARNING, "Destiny::MoveObject() %s(%u): unexpected partial state (Acc:%s Dec:%s usf:%.4f asf:%.4f) — soft recover.",
                    mySE->GetName(), mySE->GetID(), (m_accel ? "T" : "F"), (m_decel ? "T" : "F"),
                    m_userSpeedFraction, m_activeSpeedFraction);
            m_activeSpeedFraction = (m_userSpeedFraction > m_activeSpeedFraction ? m_userSpeedFraction : m_activeSpeedFraction);
            move = "soft-recover";
        }

        // Keep recovered/decel math bounded to avoid transient negative velocity (backward snap).
        if (m_activeSpeedFraction < 0.0f)
            m_activeSpeedFraction = 0.0f;
        if (m_activeSpeedFraction > 1.5f)
            m_activeSpeedFraction = 1.5f;
    ...
    bool ringOrbitKinematic = false;
    if (m_ballMode == Destiny::Ball::Mode::ORBIT && m_orbitPhase == DestinyOrbitPhase::OnRing
        && m_targetEntity.second != nullptr
        && m_orbitRadTic > 0.0f) {
        // NPCs: ring motion is kinematic (Orbit() sets phase); don't let the subwarp accel curve
        // carry a stale m_activeSpeedFraction — observers already got inconsistent vel vs position.
        if (!mySE->HasPilot()) {
            m_accel = false;
            m_decel = false;
            m_timeFraction = 1.0f;
        ...
        // Orbit() already snaps m_position onto the ring each tick; speed must match omega*r so
        // SetBallVelocity matches the path (accel curve vs fixed phase caused client jitter).
        const GPoint TpK = m_targetEntity.second->GetAuthPosition();
        const double dx = m_position.x - TpK.x;
        const double dz = m_position.z - TpK.z;
        const double rHoriz = std::max(1.0, std::sqrt(dx * dx + dz * dz));
        speed = static_cast<float>(rHoriz * static_cast<double>(m_orbitRadTic));
        ringOrbitKinematic = true;
    }

    if (m_orbiting)
        if (m_orbiting < Destiny::Ball::Orbit::TooClose && !ringOrbitKinematic) {
            // object is orbiting...set orbit speed correctly.
            speed *= m_maxOrbitSpeedFraction;
        }

    if (is_log_enabled(DESTINY__MOVE_TRACE)) {
        const char* moveTag = move;
        thread_local char orbitMoveTag[96];
        if (m_orbiting && m_orbiting < Destiny::Ball::Orbit::TooClose) {
            snprintf(orbitMoveTag, sizeof(orbitMoveTag), "%s in orbit", move);
            moveTag = orbitMoveTag;
        }
        if (m_prevSpeedFraction) {
            _log(DESTINY__MOVE_TRACE, "Destiny::MoveObject() - %s(%u) is %s at %.3f m/s (tf:%.4f asf:%.4f ps:%.2f psf:%.4f, sec: %.5f).", \
                mySE->GetName(), mySE->GetID(), moveTag, speed, m_timeFraction, m_activeSpeedFraction, m_prevSpeed, m_prevSpeedFraction, timeStamp);
        } else {
            _log(DESTINY__MOVE_TRACE, "Destiny::MoveObject() - %s(%u) is %s at %.3f m/s (tf:%.4f asf:%.4f sec: %.5f).", \
                mySE->GetName(), mySE->GetID(), moveTag, speed, m_timeFraction, m_activeSpeedFraction, timeStamp);
        }
    }
    ...
    // MoveObject runs at DynamicDestinyMs for all movers (pilots included); m_velocity is m/s — scale by wall dt.
    float posDt = 1.0f;
    {
        uint16 nominalMs(sConfig.server.DynamicDestinyMs);
        if (nominalMs < 10)
            nominalMs = 10;
        else if (nominalMs > 100)
            nominalMs = 100;
        const float nominalDt = static_cast<float>(nominalMs) * 0.001f;
        const double nowMs = GetTimeMSeconds();
        if (m_lastNpcMoveIntegrateMS != 0u) {
            posDt = static_cast<float>((nowMs - static_cast<double>(m_lastNpcMoveIntegrateMS)) * 0.001);
            if (posDt < 0.002f)
                posDt = 0.002f;
            if (posDt > 0.125f)
                posDt = 0.125f;
        ...
        m_lastNpcMoveIntegrateMS = static_cast<uint32>(nowMs);
    }

    // Position sync strategy:
    //  - PositionHack (debug): broadcast every tick to all clients.
    //  - Piloted ships: the owning client runs local movement prediction. We MUST NOT send
    //    SetBallPosition back to that client every tick (it would snap the pilot and cause
    ...
    //
    //  Bug fix: throttle must apply only to *piloted* ships. Previously the code only entered
    //  the throttle block when HasPilot(), so NPCs never set syncObservers true (except
    //  PositionHack) — no SetBallPosition was sent, clients showed static spawns while the
    ...
    // applied an extra ~1s step every tick (desync / jitter vs CmdOrbit). Approach phases use
    ...
    if (m_ballMode == Destiny::Ball::Mode::ORBIT && m_orbitPhase == DestinyOrbitPhase::OnRing) {
        SetPosition(m_position, orbitNetSync);
    } else {
...
void DestinyManager::Turn() {   // tracking within 900m for Frigates, 1k4m for BS.  05Jun17
    ...
    if (turnPercent > 0.9f) {
        if (is_log_enabled(DESTINY__TURN_TRACE))
            _log(DESTINY__TURN_TRACE, "Destiny::Turn() - clamp turnTic:%u, degRemain:%.3f, turnPercent:%.2f", m_turnTic, degrees, turnPercent);
        turnPercent = 0.9;
    }
...
void DestinyManager::ClearTurn() {
    // Sync bubble observers on turn completion (SetPosition excludes the pilot to avoid snap).
    SetPosition(m_position, sConfig.debug.PositionHack || mySE->HasPilot());
    m_lastPosBroadcast = sEntityList.GetStamp();
    m_turnTic = 0;
    m_turning = false;
    m_radians = 0.0f;
...
void DestinyManager::Follow() {
    ...
    if (m_targetEntity.second == nullptr) {
        if (m_tractored) {
            _log(DESTINY__WARNING, "DestinyManager::Follow() - %s(%u) lost tractor source. Stopping tractor motion.",
                mySE->GetName(), mySE->GetID());
            TractorBeamStop();
            return;
        }
    ...
    if (m_tractored) {
        if ((m_targetEntity.second->SysBubble() == nullptr) or m_targetEntity.second->IsDead()) {
            _log(DESTINY__WARNING, "DestinyManager::Follow() - %s(%u) tractor source %s(%u) invalid. Stopping tractor motion.",
                mySE->GetName(), mySE->GetID(), m_targetEntity.second->GetName(), m_targetEntity.second->GetID());
            TractorBeamStop();
            return;
        }
    }

    const GPoint target_point = m_targetEntity.second->GetAuthPosition();
    GVector heading(m_position, target_point);
    const double edgeDist = heading.length() - static_cast<double>(m_radius);
    if (edgeDist <= 0.0)
        m_targetDistance = 0;
    else
        m_targetDistance = static_cast<uint32>(edgeDist);
    ...
    //  by keeping their follow state, once the distance is greater than their follow distance, they will begin movement again.
        if (m_tractored) {
            ...
            DestinyManager* tdm = m_targetEntity.second->DestinyMgr();
            if ((m_targetEntity.second->IsDynamicEntity()) and (tdm != nullptr) and tdm->IsMoving()) {
                ...
                /* NPCs/drones: Stop() clears FOLLOW; AI then re-issues Follow/Orbit every tick —
                 * jerk and lost standoff rings. Idle down without dropping FOLLOW/CmdFollowBall. */
                if (mySE->IsDroneSE() || mySE->IsNPCSE())
                    SetSpeedFraction(mySE->IsDroneSE() ? 0.15f : 0.18f);
                else
                    Stop();
            }
        }
...
void DestinyManager::Orbit() {
    ...
    const uint8 orbitBandPrev = m_orbiting;
    ...
    GPoint Tp(m_targetEntity.second->GetAuthPosition());
    if (Tp.isZero() or Tp.isNaN() or Tp.isInf())
        Tp = m_targetEntity.second->GetPosition();
    if (Tp.isZero() or Tp.isNaN() or Tp.isInf()) {
        _log(DESTINY__WARNING, "Destiny::Orbit() tic - %s(%u): target %s(%u) has no valid position; stopping.",
            mySE->GetName(), mySE->GetID(), m_targetEntity.second->GetName(), m_targetEntity.second->GetID());
        Stop();
        return;
    }

    // Warp / large target motion: reacquire ring without fighting old theta.
    constexpr double kTargetJumpReacquire_m = 7500.0;
    if (m_orbitHasPrevTargetPos) {
        const double targJump = Tp.distance(m_orbitPrevTargetPos);
        if (targJump > kTargetJumpReacquire_m) {
            m_orbitPhase = DestinyOrbitPhase::Reacquire;
            m_orbitPhaseOriginMS = static_cast<uint32>(GetTimeMSeconds());
            const double jdx = m_position.x - Tp.x;
            const double jdz = m_position.z - Tp.z;
            m_orbitPhaseOffsetRad = static_cast<float>(std::atan2(jdz, jdx));
            if (is_log_enabled(DESTINY__ORBIT_TRACE))
                _log(DESTINY__ORBIT_TRACE, "orbit reacquire jump %.0fm", targJump);
        }
    }
    m_orbitPrevTargetPos = Tp;
    m_orbitHasPrevTargetPos = true;
    ...
    if (is_log_enabled(DESTINY__ORBIT_TRACE)) {
        const double tOrbit = m_orbitPhaseOriginMS
            ? (GetTimeMSeconds() - static_cast<double>(m_orbitPhaseOriginMS)) * 0.001
            : static_cast<double>(sEntityList.GetStamp() - m_stateStamp);
        _log(DESTINY__ORBIT_TRACE, "1 - %s(%u): tOrbit:%.3f, centers:%.2f, edges:%.2f, target:%u, follow:%u", \
            mySE->GetName(), mySE->GetID(), tOrbit, centers, edges, m_targetDistance, m_followDistance);
    }

    // distances checks for orbit calculations
    GPoint mPos(NULL_ORIGIN);
    float mPosAdj(0.0f);
    /* Hysteresis on TooFar / TooClose: crossing one threshold every destiny tick flipped approach vs ring
     * and caused visible pops. Separate enter/exit bands. */
    const double halfEdge = edges * 0.5;
    const double fd = static_cast<double>(m_followDistance);
    const double closeMetric = centers + m_targetDistance / 3.0;
    const double farEnterThresh = fd * 1.08;
    const double farExitThresh = fd * 0.90;
    const double closeEnterThresh = fd * 0.92;
    const double closeExitThresh = fd * 1.08;

    if (m_orbiting == Destiny::Ball::Orbit::TooFar) {
        if (halfEdge > farExitThresh) {
            MoveObject();
            return;
        }
    } else if (halfEdge > farEnterThresh
        || (m_orbitPhase == DestinyOrbitPhase::Reacquire && halfEdge > farExitThresh)) {
        m_orbiting = Destiny::Ball::Orbit::TooFar;
        // Approach / Reacquire: steer inward with subwarp integration (theta below is skipped via early return).
        ...
        if (m_orbitPhase == DestinyOrbitPhase::Reacquire)
            m_orbitPhase = DestinyOrbitPhase::Approach;
        MoveObject();
        return;
    }

    if (m_orbiting == Destiny::Ball::Orbit::TooClose) {
        if (closeMetric < closeExitThresh) {
            MoveObject();
            return;
        }
    } else if (closeMetric < closeEnterThresh) {
    ...
    // Leaving Approach bands onto the analytic φ(t) ring.
    if (orbitBandPrev == Destiny::Ball::Orbit::TooFar
        || orbitBandPrev == Destiny::Ball::Orbit::TooClose) {
        m_orbitPhaseOriginMS = static_cast<uint32>(GetTimeMSeconds());
        const double dx = m_position.x - Tp.x;
        const double dz = m_position.z - Tp.z;
        m_orbitPhaseOffsetRad = static_cast<float>(std::atan2(dz, dx));
    }

    /** Orbit angle from wall-clock seconds so motion stays smooth between 1Hz server stamps. */
    const double orbitElapsedSec = m_orbitPhaseOriginMS
        ? (GetTimeMSeconds() - static_cast<double>(m_orbitPhaseOriginMS)) * 0.001
        : static_cast<double>(sEntityList.GetStamp() - m_stateStamp);

    #define LogMacro(v) _log(DESTINY__ORBIT_TRACE, "m - " #v ": (%.3f, %.3f, %.3f)   len=%.3f", v.x, v.y, v.z, v.length())

    m_orbitPhase = DestinyOrbitPhase::OnRing;

    // Flat ring in target's horizontal plane (Y matches Tp). Old pendulum Y term mixed radians
    // with radius and fought the client's CmdOrbit path.
    float radius = m_followDistance + mPosAdj;
    float theta = -(m_orbitRadTic * static_cast<float>(orbitElapsedSec)) + m_orbitPhaseOffsetRad;
    mPos.x = radius * cos(theta);
    mPos.z = radius * sin(theta);
    mPos.y = 0;
    _log(DESTINY__ORBIT_TRACE, "4 - theta:%.5f, radius:%.3f", theta, radius);
    LogMacro(mPos);
    mPos += Tp;
    m_position = mPos;

    // Ring tangent for θ = -ωt + φ: position (r cos θ, r sin θ) in XZ → d/dt aligns with (sin θ, 0, -cos θ).
    // Do not use θ += m_orbitRadTic per ProcessState call — NPCs use ProcessHighFreqState (DynamicDestinyMs), players ~1 Hz; a fixed radian
    // bump per call desynced heading from wall-clock phase and caused visible orbit jitter.
    GVector heading(sin(theta), 0.0f, -cos(theta));
    heading.normalize();
    m_shipHeading = heading;
    m_targetPoint = m_position + (m_shipHeading * 1.0e16);
    LogMacro(heading);

    const double distFromTarget = m_position.distance(Tp);
    if (!std::isfinite(distFromTarget) || distFromTarget > 1.0e8) {
        _log(DESTINY__ERROR, "Destiny::Orbit() - %s(%u): invalid orbit distance %.0f m from target; stopping.",
            mySE->GetName(), mySE->GetID(), distFromTarget);
        Stop();
        return;
    }
    ...
    if (is_log_enabled(DESTINY__ORBIT_TRACE))
        _log(DESTINY__ORBIT_TRACE, "5 phase%u band%u orbiting centers %.2f tOrb:%.3f spd:%.2f ω·r chk:%.2f", \
            static_cast<unsigned>(m_orbitPhase), static_cast<unsigned>(orbitBandPrev), \
            m_position.distance(Tp), orbitElapsedSec, curSpeed,
            static_cast<float>(hypot((double)m_position.x - Tp.x, (double)m_position.z - Tp.z)) * std::fabs((double)m_orbitRadTic));
...
void DestinyManager::InitWarp() {
    ...
    if (not std::isfinite(m_targetDistance) or m_targetDistance <= 0.0) {
        _log(
            DESTINY__ERROR,
            "Destiny::InitWarp() - %s(%u): invalid m_targetDistance %.4f; aborting warp.",
            mySE->GetName(),
            mySE->GetID(),
            m_targetDistance
        );
    ...
    // Login warp gate should end when warp is actually initialized (not only at WarpStop).
    // Keeping LoginWarp active through the entire tunnel can cause command/state interference.
    if (mySE->HasPilot()) {
...
void DestinyManager::WarpAccel(uint16 sec_into_warp) {
    ...
    // Do NOT call SysBubble()->Remove() here — Untrack notifies other clients via
    // RemoveBall(), so observers see your ship vanish instantly instead of
    // warping away. Bubble membership is updated when WarpUpdate() reaches a new
    // bubble via Add(); ProcessWander skips entities while IsWarping().
...
void DestinyManager::WarpUpdate(double currentShipSpeed) {
    ...
    // Broadcast positions during warp so other clients interpolate smoothly
    // (otherwise only sparse updates reach observers and pilots feel jitter).
    SetPosition(m_targetPoint - (m_warpState->warp_vector * m_targetDistance), true);
...
void DestinyManager::WarpStop(double currentShipSpeed) {
    ...
    // Do not Halt() here: SetSpeedFraction(0) already leaves the ship in GOTO
    // with subwarp decel from tunnel exit speed. Halt() zeroed velocity in the
    // same tick the client still integrated warp-out — visible jitter/dsync on
    // landing (e.g. at gates). Coast to zero in MoveObject(); it calls Halt()
    // once fully stopped.
}
...
void DestinyManager::EntityRemoved(SystemEntity *pSE) {
    if (m_targetEntity.second == pSE) {
        m_targetEntity.first = 0;
        m_targetEntity.second = nullptr;
        // Always drop orbit phase flags; Stop() only runs for some modes and Halt() does not clear m_orbiting.
        ClearOrbit();
...
bool DestinyManager::IsTargetInvalid()
{
    ...
    SystemEntity* seTarg = mySE->SystemMgr()->GetSE(m_targetEntity.first);
    if (seTarg == nullptr) {
        // Our target was removed
        Stop();
        return true;
    }
    if (m_targetEntity.second == nullptr || m_targetEntity.second != seTarg)
        m_targetEntity.second = seTarg;
    ...
    DestinyManager* targDest = m_targetEntity.second->DestinyMgr();
    if (targDest != nullptr && targDest->IsWarping()) { // The target is warping
        //mySE->TargetMgr()->ClearTarget(m_targetEntity.second);
        Stop();
        return true;
    }
...
void DestinyManager::BeginMovement() {
    ...
    // Always reset the movement epoch. Skipping this for active NPCs left m_stateStamp at 0 so
    // Orbit() computed theta from (GetStamp() - 0): phase advanced by ~server uptime seconds,
    // snapping the ship to the wrong ring position each tic and fighting client CmdOrbit (rubber-banding).
    m_stateStamp = sEntityList.GetStamp();
    m_lastNpcGotoFollowBroadcastMS = 0;
    ...
    // Sync bubble observers on orbit init (SetPosition excludes the pilot to avoid snap).
    SetPosition(m_position, sConfig.debug.PositionHack || mySE->HasPilot());
    m_lastPosBroadcast = sEntityList.GetStamp();
}
...
void DestinyManager::Follow(SystemEntity* pSE, uint32 distance) {
    ...
    if (pSE == nullptr) {
        _log(DESTINY__WARNING, "Destiny::Follow() - %s(%u): null target.", mySE->GetName(), mySE->GetID());
        Stop();
        return;
    }
    ...
    m_targetPoint = pSE->GetAuthPosition();
...
void DestinyManager::WarpTo(const GPoint& where, int32 distance/*0*/, bool autoPilot/*false*/, SystemEntity* pSE/*nullptr*/) {
    ...
    GVector preLeg(m_position, where);
    const double warpLen = preLeg.length();
    const double targetDist = warpLen - static_cast<double>(m_stopDistance);
    if (mySE->IsNPCSE() or mySE->IsDroneSE()) {
        // Short leg or stop distance >= leg: m_targetDistance <= 0 → InitWarp uses log(negative) → NaN in WarpUpdate.
        if (warpLen < 1.0 or targetDist <= 1.0) {
            SetPosition(where, true);
            m_ballMode = Destiny::Ball::Mode::STOP;
            SafeDelete(m_warpState);
            return;
        }
        /* Same-grid / spawn "warp-ins" are only a few hundred km–~1 Mm. Full Ball::Mode::WARP keeps
         * IsWarping() true (target locks denied) through align+tunnel for a long time; use subwarp. */
        static const double NPC_SUBLIGHT_WARP_MAX_M = 2e6;
        if (warpLen < NPC_SUBLIGHT_WARP_MAX_M) {
            preLeg.normalize();
            GPoint landing(where);
            landing.x -= preLeg.x * static_cast<double>(m_stopDistance);
            landing.y -= preLeg.y * static_cast<double>(m_stopDistance);
            landing.z -= preLeg.z * static_cast<double>(m_stopDistance);
            GotoPoint(landing);
            SafeDelete(m_warpState);
            return;
        }
    ...
    /* GotoPoint/Follow above already called BeginMovement() against a preliminary target.
     * m_targetPoint was then adjusted for stop distance; re-sync heading, align timer (m_stateStamp),
     * and speed state to the actual warp endpoint or the server never passes the WARP alignment checks in time.
     */
    BeginMovement();
...
void DestinyManager::Orbit(SystemEntity *pSE, uint32 distance/*0*/) {
    if (pSE == nullptr) {
        _log(DESTINY__WARNING, "Destiny::Orbit() - %s(%u): null target; clearing orbit.", mySE->GetName(), mySE->GetID());
        ClearOrbit();
        return;
    }
    ...
    m_targetEntity.second = pSE;
    m_targetPoint = pSE->GetAuthPosition();
    if (m_targetPoint.isZero() or m_targetPoint.isNaN() or m_targetPoint.isInf())
        m_targetPoint = pSE->GetPosition();
    if (m_targetPoint.isZero() or m_targetPoint.isNaN() or m_targetPoint.isInf()) {
        _log(DESTINY__WARNING, "Destiny::Orbit() - %s(%u): target %s(%u) has no valid position; clearing orbit.",
    ...
    OrbitKinematics::Params ok;
    ok.cmd_m = static_cast<double>(distance);
    ok.self_radius_m = m_radius;
    ok.target_radius_m = Tr;
    ok.max_ship_speed_mps = m_maxShipSpeed;
    ok.ship_agility = m_shipAgility;

    const OrbitKinematics::Result kin = OrbitKinematics::ComputeLegacyCompatible(ok);
    {
        constexpr double fdMax = 2147483000.0;
        double rEnc = kin.r_follow_m;
        if (!std::isfinite(rEnc) || rEnc < 50.0)
            rEnc = distance + Tr + m_radius;
        rEnc = std::max(50.0, std::min(rEnc, fdMax));
        m_followDistance = static_cast<uint32>(rEnc);
    }

    double velocity = kin.tangential_speed_mps;
    if (!std::isfinite(velocity) || velocity < 0.0)
        velocity = m_maxShipSpeed * ((static_cast<double>(distance) / std::max(1u, m_followDistance)) + 0.065);
    m_maxOrbitSpeedFraction = static_cast<float>(velocity / std::max(1e-6, (double)m_maxShipSpeed));

    m_orbitTime = kin.orbit_period_s;
    m_orbitRadTic = static_cast<float>(kin.omega_rad_s);
    m_orbitPhaseOriginMS = static_cast<uint32>(GetTimeMSeconds());
    m_orbitPhaseOffsetRad = static_cast<float>(EvE::Trig::Pi2 * static_cast<double>(mySE->GetID() % 997u) / 997.0);
    m_orbitPhase = DestinyOrbitPhase::Approach;
    m_orbitHasPrevTargetPos = false;

    if (is_log_enabled(DESTINY__ORBIT_TRACE))
        _log(DESTINY__ORBIT_TRACE, "%s(%u) - Orbit Data - r_follow:%u v:%.2f ω:%.8f Osf:%.3f tgtDist:%.2f orbT:%.1f",
            mySE->GetName(), mySE->GetID(), m_followDistance, velocity, kin.omega_rad_s, m_maxOrbitSpeedFraction,
            m_targetDistance, m_orbitTime);

    if (m_followDistance == 0) {
        _log(DESTINY__ERROR, "%s(%u) - FollowDistance is 0.", mySE->GetName(), mySE->GetID());
        m_followDistance = static_cast<uint32>(m_targetDistance + Tr + m_radius); // fudge something here.  will have to fix later, but this is close enough
    }

    CmdOrbit du;
        du.entityID = mySE->GetID();
        du.orbitEntityID = pSE->GetID();
        du.distance = (m_followDistance > 0 ? static_cast<int32>(m_followDistance) : static_cast<int32>(m_targetDistance));
    PyTuple *up = du.Encode();
    SendSingleDestinyUpdate(&up);   // consumed
}
...
void DestinyManager::SetPosition(const GPoint &pt, bool update /*false*/) {
    ...
    // Never teleport to a random map point on bad input. Drones can still receive (0,0,0)
    // from broken orbit math when the controller position was stale; recover from inventory.
    GPoint usePt(pt);
    if (usePt.isZero() or usePt.isNaN() or usePt.isInf()) {
        if (mySE->IsDroneSE()) {
            const GPoint ip(mySE->GetPosition());
            if (ip.isNotZero() and !ip.isNaN() and !ip.isInf()) {
                usePt = ip;
                _log(DESTINY__MESSAGE, "Destiny::SetPosition() - drone %s(%u): invalid snap; using item position %.2f,%.2f,%.2f",
                    mySE->GetName(), mySE->GetID(), usePt.x, usePt.y, usePt.z);
            }
        }
        if (usePt.isZero() or usePt.isNaN() or usePt.isInf()) {
            _log(DESTINY__ERROR, "Destiny::SetPosition() - rejecting invalid point for %s(%u): zero=%s nan=%s inf=%s. Keeping current position %.2f,%.2f,%.2f",
                mySE->GetName(), mySE->GetID(),
    ...
    if (mySE->IsPOSSE() or mySE->IsProbeSE() or update) {
        // For piloted ships, bundle velocity + heading with the position snap.
        //
        // Rationale: observer clients run their own destiny sim per bubble ball.
        // A lone SetBallPosition tells the client "teleport to X" but leaves the
        // ball with whatever velocity / heading it last cached. With per-tick (or
        // every-3-tick) position broadcasts that caused observers to see other
        // ships visually teleport each tick while staying pinned to their default
        // orientation ("facing straight down"). Sending SetBallVelocity +
        // CmdGotoDirection alongside lets the client smoothly interpolate between
        // snaps and auto-orient the model to the flight vector. See DestinyManager
        // ::Undock() for the same packet triplet used on entry to flight.
        const bool pilotedShip = mySE->HasPilot() and (mySE->SysBubble() != nullptr);

        if (pilotedShip) {
            std::vector<PyTuple*> updates;
            SetBallPosition pos;
                pos.entityID = mySE->GetID();
                pos.x = m_position.x;
                pos.y = m_position.y;
                pos.z = m_position.z;
            updates.push_back(pos.Encode());

            // Only publish velocity / heading when we actually have flight data.
            // Stationary ships should snap cleanly without a stale heading vector
            // that would leave observers rendering them drifting at zero speed.
            const double vlen2 = m_velocity.x * m_velocity.x
                               + m_velocity.y * m_velocity.y
                               + m_velocity.z * m_velocity.z;
            // Match non-pilot threshold: sub-1 m/s flight still needs vel+heading so observers do not
            // extrapolate from position-only snaps (same desync class as NPC orbit creep).
            if (vlen2 > 1e-6) {
                SetBallVelocity vel;
                    vel.entityID = mySE->GetID();
                    vel.x = m_velocity.x;
                    vel.y = m_velocity.y;
                    vel.z = m_velocity.z;
                updates.push_back(vel.Encode());

                const double hlen2 = m_shipHeading.x * m_shipHeading.x
                                   + m_shipHeading.y * m_shipHeading.y
                                   + m_shipHeading.z * m_shipHeading.z;
                if (hlen2 > 0.0001) {
                    CmdGotoDirection dir;
                        dir.entityID = mySE->GetID();
                        dir.x = m_shipHeading.x;
                        dir.y = m_shipHeading.y;
                        dir.z = m_shipHeading.z;
                    updates.push_back(dir.Encode());
                }
            }

            // Fan out exclusive of the pilot (who runs the authoritative sim client-side).
            SystemBubble* pBubble = mySE->SysBubble();
            for (auto& packet : updates) {
                pBubble->BubblecastDestinyUpdateExclusive(&packet, "destiny", mySE);
                PySafeDecRef(packet);
            }

            // Periodic server-authoritative correction for the pilot as well.
            // The client runs local prediction, but across long sessions prediction drift
            ...
            const uint32 nowMS = static_cast<uint32>(GetTimeMSeconds());
            if ((nowMS - m_lastSelfSyncMS) >= 200) {
                m_lastSelfSyncMS = nowMS;

                SetBallPosition selfPos;
                    selfPos.entityID = mySE->GetID();
                    selfPos.x = m_position.x;
                    selfPos.y = m_position.y;
                    selfPos.z = m_position.z;
                PyTuple* up = selfPos.Encode();
                SendSingleDestinyUpdate(&up, true);   // consumed
            }
        } else {
            // Non-piloted movers (NPCs/drones/missiles) also benefit from velocity/heading
            // packets so clients interpolate motion instead of stepping position snaps.
            //
            // Ring orbit is kinematically curved ( ω≠0 ). Throttling SetBallPosition (~150ms)
            // while still broadcasting velocity each tick made observers extrapolate straight tangents
            // until the next snap → periodic jumps toward true orbit arc — visually teleport/jitter.
            // Align snaps with high-frequency destiny (~50ms); CmdOrbit still primes observer locals.
            ...
            // NPCs/drones often move at <1 m/s (orbit settle, follow creep). Skipping velocity below
            // 1 m/s left clients with position-only snaps and overview speed 0 while the model still moved.
            const double vlen2 = m_velocity.x * m_velocity.x
                               + m_velocity.y * m_velocity.y
                               + m_velocity.z * m_velocity.z;
            if (vlen2 > 1e-6) {
...
void DestinyManager::SetRadius(double radius, bool update /*false*/) {
    if (is_log_enabled(DESTINY__TRACE))
        _log(DESTINY__TRACE, "Destiny::SetRadius() called by %s(%u)", mySE->GetName(), mySE->GetID());
...
void DestinyManager::MakeMissile(Missile* pMissile) {
    ...
    m_targetPoint = GPoint(pTarget->GetAuthPosition());
...
{
    Halt();
    // Allow UpdateNewShip() to run again after system change — same hull, but
    // stale m_hasSentShipUpdates skipped UpdateShipVariables / slim refresh so
    // warp-related HUD (e.g. bubble collapse / ship stats) disappeared post-jump.
    m_hasSentShipUpdates = false;
    if (showCloak) {
        m_cloaked = true;
    }
...
void DestinyManager::Cloak(ActiveModule* srcMod/*nullptr*/) {
    if (m_cloaked)
        return;
    m_cloaked = true;
    m_cloakMod = srcMod;
    m_preCloakMaxSpeed = m_maxShipSpeed;

    // Apply the cloak velocity bonus from the module, if any. AttrCloakVelocityBonus
    // is expressed as a signed percentage (e.g. -75 for a standard cloak, 0 for
    // covert ops). Fall back to the hard EVE default of -75% if the module does
    // not publish the attribute, so buggy/incomplete data still produces a
    // sensible slowdown instead of letting ships cloak at full speed.
    if (srcMod != nullptr) {
        float bonus = -75.0f;
        ModuleItemRef modRef(srcMod->GetSelf());
        if (modRef.get() != nullptr && modRef->HasAttribute(AttrCloakVelocityBonus))
            bonus = modRef->GetAttribute(AttrCloakVelocityBonus).get_float();
        const float scale = 1.0f + (bonus / 100.0f);
        // Guard against bad attr data flipping sign / producing NaN
        if (scale <= 0.0f || !std::isfinite(scale)) {
            m_maxShipSpeed = 1.0f;
        } else {
            m_maxShipSpeed = m_preCloakMaxSpeed * scale;
        }
        // Clamp current active fraction so the ship physically slows down this
        // tick — m_activeSpeedFraction is fraction of (new) m_maxShipSpeed so
        // anything > scale would re-accelerate us right back to pre-cloak speed.
        if (m_activeSpeedFraction > scale)
            m_activeSpeedFraction = scale;
        if (m_userSpeedFraction > scale)
            m_userSpeedFraction = scale;
    ...
    // Kick the client's velocity state so it picks up the new cap immediately
    // (matches the pattern used by WebbedMe / SpeedBoost). Skip the call for a
    // stationary ship — SetSpeedFraction(0, true) would flip the ball mode out
    // of STOP into GOTO and fly the ship toward a stale target point.
    if (m_userSpeedFraction > 0.001f || IsMoving())
        SetSpeedFraction(m_userSpeedFraction, true);

    SendCloakFx(true, srcMod != nullptr);
    if (mySE->SysBubble() != nullptr)
        mySE->SysBubble()->RemoveExclusive(mySE);
    m_lastCloakProxCheck = sEntityList.GetStamp();
}
...
void DestinyManager::UnCloak() {
    if (!m_cloaked)
        return;
    m_cloaked = false;
    const bool wasModule = (m_cloakMod != nullptr);

    // Restore max velocity. Prefer the cached pre-cloak value (captures any
    // active AB/MWD/web state in effect when the cloak engaged); if we never
    // saved one (jump cloak, GM command) fall back to the ship's base
    // AttrMaxVelocity so we don't leave stale slow data on the entity.
    if (m_preCloakMaxSpeed > 0.0f) {
        m_maxShipSpeed = m_preCloakMaxSpeed;
    } else if (mySE->GetSelf().get() != nullptr) {
        m_maxShipSpeed = mySE->GetSelf()->GetAttribute(AttrMaxVelocity).get_float();
    }
    m_preCloakMaxSpeed = 0.0f;
    ...
    // Same stationary-ship guard as Cloak() — avoid flipping STOP into GOTO.
    if (m_userSpeedFraction > 0.001f || IsMoving())
        SetSpeedFraction(m_userSpeedFraction, true);
    SendCloakFx(false, wasModule);
    if (mySE->SysBubble() != nullptr)
        mySE->SysBubble()->AddBallExclusive(mySE);

    // If an active cloak module triggered this cloak, cancel its cycle so the
    // module UI snaps out of "active" and the next activation can re-engage.
    // Only fire AbortCycle when the module is still in its Activated state to
    // avoid re-entering DeactivateCycle() (which called us in the first place
    // via AbortCycle -> DeactivateCycle -> switch -> UnCloak). Clearing
    // m_cloakMod before the call also prevents any nested UnCloak from
    // re-triggering the same abort if the flow unwinds unexpectedly.
    if (m_cloakMod != nullptr) {
        ActiveModule* mod = m_cloakMod;
        m_cloakMod = nullptr;
        if (mod->GetModuleState() == Module::State::Activated)
            mod->AbortCycle();
    }
}

void DestinyManager::CheckCloakProximity() {
    if (!m_cloaked)
        return;
    // Only module-driven cloaks are subject to proximity decloak. Jump cloak
    // has its own timer-driven lifecycle in Client::ProcessClient() and admin
    // cloaks should not auto-break when ships approach.
    if (m_cloakMod == nullptr)
        return;
    ...
    // EVE-canonical decloak range is 2000m between hulls. A ship decloaks when
    // any other dynamic entity gets within 2000m + our radius + their radius.
    // We scan the bubble (small, spatially local) instead of the whole system.
    static const double kDecloakRange = 2000.0;

    thread_local std::map<uint32, SystemEntity*> entities;
    entities.clear();
    pBubble->GetAllEntities(entities);

    for (auto& kv : entities) {
        SystemEntity* pSE = kv.second;
        if (pSE == nullptr)
            continue;
        if (pSE == mySE)
            continue;
        // Only dynamic, targetable ship-like entities break cloaks. Static
        // structures, asteroids and wrecks do not (CCP rule).
        if (!(pSE->IsShipSE() || pSE->IsNPCSE() || pSE->IsDroneSE()))
            continue;
        if (pSE->IsInvul() || pSE->IsDead())
            continue;

        const double dist = m_position.distance(pSE->GetAuthPosition());
        const double trigger = kDecloakRange + m_radius + pSE->GetRadius();
        if (dist < trigger) {
            _log(DESTINY__MOVE_TRACE,
                 "Destiny::CheckCloakProximity() - %s(%u) decloaked by %s(%u) at %.1fm (trigger %.1fm).",
                 mySE->GetName(), mySE->GetID(), pSE->GetName(), pSE->GetID(), dist, trigger);
            UnCloak();
            return;
        }
    }
...
void DestinyManager::TractorBeamStart(SystemEntity* pShipSE, EvilNumber speed)
{
    ...
    m_targetPoint = pShipSE->GetAuthPosition();
    GVector moveVector(m_position, m_targetPoint);
...
void DestinyManager::SendSetState() const {
    ...
    /* Push recharge attrs again now that Destiny queues flush — fixes client godma tau after login SetState. */
    if (mySE->IsShipSE()) {
        ShipItemRef hull(static_cast<ShipSE*>(mySE)->GetShipItemRef());
        if (hull.get() != nullptr)
            hull->SanitizeSimulatorRechargeAttributes();
    }
}
