...
{       // NOTE this is for players and CAN throw (client calls this inside try/catch block)
    if (tSE == nullptr) {
        codelog(TARGET__ERROR, "StartTargeting() called with null target by %s(%u).", mySE->GetName(), mySE->GetID());
        return false;
    }
    if (!mySE->HasPilot()) {
...
{       // NOTE  this is for npcs
    if (tSE == nullptr) {
        codelog(TARGET__ERROR, "NPC StartTargeting() called with null target by %s(%u).", mySE->GetName(), mySE->GetID());
        return false;
    }
    //first make sure they are not already in the list
    if (m_targets.find(tSE) != m_targets.end()) {
...
void TargetManager::ClearTargets(bool notify/*true*/) {
    ...
    for (auto cur : m_targets) {
        if ((cur.first == nullptr) or (cur.second == nullptr))
            continue;
...
float TargetManager::TimeToLock ( ShipItemRef sRef, SystemEntity* tSE ) const {
    ...
    if ( tSE->GetSelf().get() != nullptr )
        ...
        double asinhSig = asinh(sigRad);
        if (asinhSig < 0.0001)
            asinhSig = 0.0001;
        float time = ( 40000 /(scanRes * std::pow(asinhSig, 2)));   // higher scan res means faster lock time.
