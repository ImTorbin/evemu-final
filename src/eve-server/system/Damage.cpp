...
bool SystemEntity::ApplyDamage(Damage &d) {
    ...
    // Hard guards: callers must never pass these, but combat paths (NPC missiles,
    // stacking edge cases, race at warp-in) have produced null refs in the wild.
    if (d.srcSE == nullptr) {
        _log(DAMAGE__ERROR, "ApplyDamage(%s): null damage source SE — ignoring.", GetName());
        return false;
    }
    if (d.weaponRef.get() == nullptr) {
        _log(DAMAGE__ERROR, "ApplyDamage(%s): null weaponRef — ignoring.", GetName());
        return false;
    }
    ...
    if (shield_damage <= available_shield) {
        ...
        */
        ...
        // Zero / denormal shield_damage would divide-by-zero or explode NaNs through the armor/hull chain.
        if (shield_damage > 1e-12f)
        ...
        if (armor_damage <= available_armor) {
            ...
            if (HasPilot()) {
                ShipSE* pHull = GetShipSE();
                if (pHull != nullptr)
                    pHull->DamageRandModule(sConfig.server.ModuleDamageChance);    // config option for random module damage chance
            }
        }
    }
    ...
    if (killed) {
        ...
        if (d.srcSE->HasPilot()) {
            ...
            bool banked = false;
            tuple->SetItem(0, new PyString("OnDamageMessage"));
            if (d.weaponRef.get() != nullptr && d.weaponRef->IsModuleItem()) {
                ShipSE* attShip = d.srcSE->GetShipSE();
                ShipItemRef shipRef(attShip != nullptr ? attShip->GetShipItemRef() : ShipItemRef(nullptr));
                if (shipRef.get() != nullptr) {
                    GenericModule* pMod = shipRef->GetModule(d.weaponRef->flag());
