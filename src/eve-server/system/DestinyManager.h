...
public:
    ...
    /** True if the ship has meaningful velocity or commanded / integrated speed.
     *  Using only m_timeFraction missed valid motion (tf can be 0 briefly) and
     *  matched stale states — STOP/Stop() interacted badly with subwarp flight. */
    bool IsMoving();
    ...
    /** True for entire warp sequence including pre-InitWarp alignment (m_ballMode WARP, m_warpState null).
     *  If this only checked m_warpState, a second WarpTo during align could run and corrupt the first warp. */
    bool IsWarping()                                    { return (m_ballMode == Destiny::Ball::Mode::WARP); }
    ...
    /** Activate cloaking. When called from an active cloaking device module
     *  (srcMod != nullptr) the ship's max velocity is throttled by the
     *  module's AttrCloakVelocityBonus and the module is tracked so
     *  auto-decloak events (proximity, warp, damage, etc.) can abort its
     *  cycle. Passing nullptr preserves the original behaviour used by the
     *  jump-cloak path and admin/GM commands. */
    void Cloak(ActiveModule* srcMod = nullptr);
    void UnCloak();
    /** Scans the current bubble for ships/NPCs/drones within the decloak
     *  proximity radius; if any are found, breaks the cloak (and the
     *  tracked cloak module cycle). Cheap no-op for non-cloaked entities. */
    void CheckCloakProximity();
...
protected:
    ...
    // Tracked cloaking device module (if any). Owned by ShipItem's ModuleManager,
    // we just keep a non-owning pointer so UnCloak() can abort its cycle when
    // the cloak is broken externally (proximity, warp, damage, etc.).
    ActiveModule* m_cloakMod;
    // Saved max velocity before cloak speed throttle was applied, so UnCloak()
    // can restore the prior m_maxShipSpeed without double-applying bonuses.
    float m_preCloakMaxSpeed;
