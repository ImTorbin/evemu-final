...
{   // working
    ...
    if (notif.data->items.size()) {
        sEntityList.CorpNotify(notif.key, Notify::Types::CorpNews, "OnCorporationChanged", "corpid", notif.Encode());
        call.client->SendNotification("OnCorporationChanged", "clientID", notif.Encode(), false);
    }
...
PyResult CorpRegistryBound::UpdateTitle(PyCallArgs &call,
    ...
    if (m_db.UpdateTitle(m_corpID, args, updates)) {
        ...
        sEntityList.CorpNotify(m_corpID, Notify::Types::CorpNews, "OnTitleChanged", "corpid", change.Encode());
    }
