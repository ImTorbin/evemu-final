        ...
        for (auto wingID : wingIDs) {
            ...
            for (auto squadID : squadIDs) {
                ...
                for (auto cur : sData.members) { // SC is a member
                    if ((cur.second == nullptr) or !cur.second->IsInSpace() or (cur.second->GetShipSE() == nullptr))
                        continue;

                    BoostData memberBoost = BoostData();
                    if (sboost and (sData.booster != nullptr) and (cur.second->GetSystemID() == sData.booster->GetSystemID()))
                        memberBoost = sData.boost;

                    memberUpdateMap.insert_or_assign(cur.second->GetShipSE(), memberBoost);
                }
            }
        }
    } else if (!wing.empty()) {
...
bool FleetService::GetFleetWarpTargets(Client* issuer, std::vector<Client*>& out)
{
    out.clear();
    if (issuer == nullptr)
        return false;

    const uint32 fleetID = issuer->GetFleetID();
    if (!IsFleetID(fleetID))
        return false;

    const int8 role = issuer->GetFleetRole();
    if (role != Fleet::Role::FleetLeader
     && role != Fleet::Role::WingLeader
     && role != Fleet::Role::SquadLeader)
        return false;

    // issuer must be in space to have a grid/system scope to warp from
    if (!issuer->IsInSpace() || issuer->GetShipSE() == nullptr
    ||  issuer->GetShipSE()->SystemMgr() == nullptr)
        return false;

    const uint32 scopeSystemID = issuer->GetShipSE()->SystemMgr()->GetID();
    const int32 issuerWingID   = issuer->GetWingID();
    const int32 issuerSquadID  = issuer->GetSquadID();
    const uint32 issuerCharID  = issuer->GetCharacterID();

    auto range = m_fleetMembers.equal_range(fleetID);
    for (auto itr = range.first; itr != range.second; ++itr) {
        Client* cur = itr->second;
        if (cur == nullptr || cur == issuer)
            continue;
        if (cur->GetCharacterID() == issuerCharID)
            continue;
        if (!cur->IsInSpace() || cur->GetShipSE() == nullptr
        ||  cur->GetShipSE()->SystemMgr() == nullptr)
            continue;
        if (cur->GetShipSE()->SystemMgr()->GetID() != scopeSystemID)
            continue;

        // narrow by role scope
        if (role == Fleet::Role::WingLeader) {
            if (cur->GetWingID() != issuerWingID)
                continue;
        } else if (role == Fleet::Role::SquadLeader) {
            if (cur->GetSquadID() != issuerSquadID)
                continue;
        }
...
std::string FleetService::GetBoosterData(uint32 fleetID, uint16& length)
{
    ...
    if ((fData.creator != nullptr) and (fData.creator->GetChar().get() != nullptr))
        creatorName = fData.creator->GetChar()->itemName();
    str << "<color=teal>" << fData.name << "  Created By: " << creatorName;
    str << "  Members: " << std::to_string(m_fleetMembers.count(fleetID)) << "</color><br>"; //54
    length += 54;
    length += fData.name.size();
    length += creatorName.size();
