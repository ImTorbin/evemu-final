...
PyResult FleetBound::Invite(PyCallArgs &call, PyInt* characterID, std::optional<PyInt*> wingID, std::optional <PyInt*> squadID, std::optional <PyInt*> role) {
...
PyResult FleetBound::UpdateMemberInfo(PyCallArgs &call, std::optional <PyInt*> shipTypeID) {
    ...
    if (!IsFleetID(pChar->fleetID()))
        return nullptr;

    bool updateFleet = true;
    std::list<int32> wing;
    std::list<int32> squad;
    if (IsWingID(pChar->wingID())) {
        updateFleet = false;
        wing.emplace_back(pChar->wingID());
    }
    if (IsSquadID(pChar->squadID())) {
        updateFleet = false;
        squad.emplace_back(pChar->squadID());
    }

    // This RPC fires when member info changes (ship/location/session state).
    // Recompute and reapply fleet/wing/squad boosts so gang effects stay in sync.
    sFltSvc.UpdateBoost(pChar->fleetID(), updateFleet, wing, squad);
