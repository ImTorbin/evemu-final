...
PyRep *MarketDB::GetStationAsks(uint32 stationID) {
    const double beginMs = GetTimeMSeconds();
    DBQueryResult res;
    if (!sDatabase.RunQuery(res,
        "SELECT"
        "    typeID, MIN(price) AS price, SUM(volRemaining) AS volRemaining, stationID "
        " FROM mktOrders "
        " WHERE stationID=%u AND bid=%u"
        " GROUP BY typeID, stationID",  stationID, Market::Type::Sell))
    ...
    const uint32 rowCount = res.GetRowCount();
    PyRep* out = DBResultToIndexRowset(res, "typeID");
    const double elapsedMs = GetTimeMSeconds() - beginMs;
    if (elapsedMs > 250.0) {
        _log(MARKET__WARNING, "GetStationAsks(%u) fetched %u rows in %.2fms", stationID, rowCount, elapsedMs);
    } else if (is_log_enabled(MARKET__DB_TRACE)) {
        _log(MARKET__DB_TRACE, "GetStationAsks(%u) fetched %u rows in %.2fms", stationID, rowCount, elapsedMs);
    }
    return out;
}
...
PyRep *MarketDB::GetSystemAsks(uint32 solarSystemID) {
    ...
    if (!sDatabase.RunQuery(res,
        "SELECT"
        "    b.typeID, b.price, SUM(o.volRemaining) AS volRemaining, MIN(o.stationID) AS stationID "
        " FROM mktOrders AS o"
        " INNER JOIN ("
        "   SELECT typeID, MIN(price) AS price"
        "   FROM mktOrders"
        "   WHERE solarSystemID=%u AND bid=%u"
        "   GROUP BY typeID"
        " ) AS b ON b.typeID = o.typeID AND b.price = o.price"
        " WHERE o.solarSystemID=%u AND o.bid=%u"
        " GROUP BY b.typeID, b.price",
        solarSystemID, Market::Type::Sell, solarSystemID, Market::Type::Sell))
    ...
    const double elapsedMs = GetTimeMSeconds() - beginMs;
    if (elapsedMs > 250.0) {
        _log(MARKET__WARNING, "GetSystemAsks(%u) fetched %u rows in %.2fms", solarSystemID, rowCount, elapsedMs);
    } else if (is_log_enabled(MARKET__DB_TRACE)) {
        _log(MARKET__DB_TRACE, "GetSystemAsks(%u) fetched %u rows in %.2fms", solarSystemID, rowCount, elapsedMs);
    }
    return out;
}
...
PyRep *MarketDB::GetRegionBest(uint32 regionID) {
    ...
    if (!sDatabase.RunQuery(res,
        ...
        "   GROUP BY typeID"
        " ) AS b ON b.typeID = o.typeID AND b.price = o.price"
        " WHERE o.regionID=%u AND o.bid=%u"
        " GROUP BY b.typeID, b.price",
        regionID, Market::Type::Sell, regionID, Market::Type::Sell))
    ...
    const double elapsedMs = GetTimeMSeconds() - beginMs;
    if (elapsedMs > 250.0) {
        _log(MARKET__WARNING, "GetRegionBest(%u) fetched %u rows in %.2fms", regionID, rowCount, elapsedMs);
    } else if (is_log_enabled(MARKET__DB_TRACE)) {
        _log(MARKET__DB_TRACE, "GetRegionBest(%u) fetched %u rows in %.2fms", regionID, rowCount, elapsedMs);
    }
    return out;
}
