...
PyResult MarketProxyService::GetStationAsks(PyCallArgs &call) {
    ...
    const double startMs = GetTimeMSeconds();
    PyResult result = GetCachedAsks("GetStationAsks_", stationID, true);
    const double elapsedMs = GetTimeMSeconds() - startMs;
    if (elapsedMs > 250.0) {
        _log(MARKET__WARNING, "GetStationAsks(%u) handled in %.2fms", stationID, elapsedMs);
    } else if (is_log_enabled(MARKET__DEBUG)) {
        _log(MARKET__DEBUG, "GetStationAsks(%u) handled in %.2fms", stationID, elapsedMs);
    }
    return result;
}

PyResult MarketProxyService::GetSystemAsks(PyCallArgs &call) {
    const uint32 systemID = call.client->GetSystemID();
    const double startMs = GetTimeMSeconds();
    PyResult result = GetCachedAsks("GetSystemAsks_", systemID, false);
    const double elapsedMs = GetTimeMSeconds() - startMs;
    if (elapsedMs > 250.0) {
        _log(MARKET__WARNING, "GetSystemAsks(%u) handled in %.2fms", systemID, elapsedMs);
    } else if (is_log_enabled(MARKET__DEBUG)) {
        _log(MARKET__DEBUG, "GetSystemAsks(%u) handled in %.2fms", systemID, elapsedMs);
    }
    return result;
}

PyResult MarketProxyService::GetRegionBest(PyCallArgs &call) {
    const uint32 regionID = call.client->GetRegionID();
    const double startMs = GetTimeMSeconds();
    PyResult result = GetCachedRegionBest(regionID);
    const double elapsedMs = GetTimeMSeconds() - startMs;
    if (elapsedMs > 250.0) {
        _log(MARKET__WARNING, "GetRegionBest(%u) handled in %.2fms", regionID, elapsedMs);
    } else if (is_log_enabled(MARKET__DEBUG)) {
        _log(MARKET__DEBUG, "GetRegionBest(%u) handled in %.2fms", regionID, elapsedMs);
    }
    return result;
}
...
PyResult MarketProxyService::GetCachedAsks(const char* methodPrefix, uint32 scopeID, bool stationScope) {
    std::string method_name(methodPrefix);
    method_name += std::to_string(scopeID);
    ObjectCachedMethodID method_id(GetName().c_str(), method_name.c_str());

    uint32 expiryMs = 0;
    if (stationScope) {
        auto itr = m_stationAsksExpiryMs.find(scopeID);
        if (itr != m_stationAsksExpiryMs.end())
            expiryMs = itr->second;
    } else {
        auto itr = m_systemAsksExpiryMs.find(scopeID);
        if (itr != m_systemAsksExpiryMs.end())
            expiryMs = itr->second;
    }

    const uint32 nowMs = static_cast<uint32>(GetTimeMSeconds());
    const bool cacheValid = (expiryMs >= nowMs) && m_cache->IsCacheLoaded(method_id);
    if (!cacheValid) {
        m_cache->InvalidateCache(method_id);

        PyRep* result = stationScope
            ? MarketDB::GetStationAsks(scopeID)
            : MarketDB::GetSystemAsks(scopeID);

        if (result == nullptr) {
            _log(MARKET__DB_ERROR, "Failed to build %s cache for scope %u", methodPrefix, scopeID);
            result = PyStatic.NewNone();
        }

        m_cache->GiveCache(method_id, &result);
        if (stationScope) {
            m_stationAsksExpiryMs[scopeID] = nowMs + kAsksCacheTTLms;
        } else {
            m_systemAsksExpiryMs[scopeID] = nowMs + kAsksCacheTTLms;
        }
    }

    return m_cache->MakeObjectCachedMethodCallResult(method_id);
}

PyResult MarketProxyService::GetCachedRegionBest(uint32 regionID) {
    std::string method_name("GetRegionBest_");
    method_name += std::to_string(regionID);
    ObjectCachedMethodID method_id(GetName().c_str(), method_name.c_str());

    uint32 expiryMs = 0;
    auto itr = m_regionBestExpiryMs.find(regionID);
    if (itr != m_regionBestExpiryMs.end())
        expiryMs = itr->second;
    ...
    if (!cacheValid) {
        ...
        PyRep* result = MarketDB::GetRegionBest(regionID);
        if (result == nullptr) {
            _log(MARKET__DB_ERROR, "Failed to build GetRegionBest cache for region %u", regionID);
            result = PyStatic.NewNone();
        }

        m_cache->GiveCache(method_id, &result);
        m_regionBestExpiryMs[regionID] = nowMs + kRegionBestCacheTTLms;
    }

    return m_cache->MakeObjectCachedMethodCallResult(method_id);
}

void MarketProxyService::InvalidateAsksCache() {
    // Lazy invalidation: clear local TTL trackers so next read refreshes object cache entries.
    m_stationAsksExpiryMs.clear();
    m_systemAsksExpiryMs.clear();
    m_regionBestExpiryMs.clear();
}
