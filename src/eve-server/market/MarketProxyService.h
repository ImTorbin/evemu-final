...
private:
    ...
    static constexpr uint32 kAsksCacheTTLms = 30000;
    static constexpr uint32 kRegionBestCacheTTLms = 30000;
    std::unordered_map<uint32, uint32> m_systemAsksExpiryMs;
    std::unordered_map<uint32, uint32> m_stationAsksExpiryMs;
    std::unordered_map<uint32, uint32> m_regionBestExpiryMs;

    PyResult GetCachedAsks(const char* methodPrefix, uint32 scopeID, bool stationScope);
    PyResult GetCachedRegionBest(uint32 regionID);
    void InvalidateAsksCache();
};
