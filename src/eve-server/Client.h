...
private:
    /** DoDestinyUpdate.waitForBubble: client (Michelle) errors if true before ballpark exists — gate on SetState. */
    bool DestinyUpdatesWaitForBubble() const              { return m_bubbleWait && m_setStateSent; }
...
public:
    ...
    int8 GetPlayerState() const                         { return m_clientState; }
